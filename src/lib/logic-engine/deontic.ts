...
export function validateClaim(
  ...
  const sourceIds = [...new Set(claimedSourceIds.map((id) => id.trim()).filter(Boolean))];

  if (!claim) {
    return rejected(sourceIds, "EMPTY_CLAIM", "İddia metni boş olamaz.", "Claim cannot be empty.");
  }
  if (sourceIds.length === 0) {
    return rejected(sourceIds, "MISSING_SOURCE_ID", "source_id zorunludur; iddia reddedildi.", "source_id is required; claim rejected.");
  }

  const byId = new Map(rules.map((rule) => [rule.id, rule]));
  const unknown = sourceIds.find((id) => !byId.has(id));
  if (unknown) {
    return rejected(sourceIds, "UNKNOWN_SOURCE_ID", `Kayıtlı olmayan source_id: ${unknown}`, `Unknown source_id: ${unknown}`);
  }

  const inactive = sourceIds.find((id) => !byId.get(id)!.isActive);
  if (inactive) {
    return rejected(sourceIds, "INACTIVE_SOURCE_ID", `Pasif source_id: ${inactive}`, `Inactive source_id: ${inactive}`);
  }
  ...
  if (invalid) {
    return rejected(sourceIds, "INVALID_FORMAL_EXPRESSION", `Geçersiz deontik ifade: ${invalid}`, `Invalid deontic expression: ${invalid}`);
  }
