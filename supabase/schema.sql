...
CREATE TABLE IF NOT EXISTS approval_logs (
  ...
  old_value TEXT, new_value TEXT, performed_by UUID NOT NULL REFERENCES profiles(id), note TEXT, created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS subscriptions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), user_id UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
  tier TEXT NOT NULL CHECK (tier IN ('free', 'private')), status TEXT NOT NULL DEFAULT 'active' CHECK (status IN ('active', 'cancelled', 'expired', 'pending_payment')),
  monthly_fee NUMERIC(10,2) NOT NULL DEFAULT 0, starts_at TIMESTAMPTZ DEFAULT NOW(), ends_at TIMESTAMPTZ, created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS user_files (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), user_id UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
  original_name TEXT NOT NULL, storage_path TEXT NOT NULL, mime_type TEXT, size_bytes BIGINT,
  category TEXT DEFAULT 'general' CHECK (category IN ('general', 'case', 'precedent', 'evidence', 'other')), is_private BOOLEAN DEFAULT TRUE, created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS cases (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), uyap_id TEXT UNIQUE,
  court_type TEXT NOT NULL CHECK (court_type IN ('Yargitay', 'Danistay', 'AYM', 'Asliye', 'Idare', 'other')),
  decision_number TEXT NOT NULL, decision_date DATE NOT NULL, parties JSONB, full_text TEXT NOT NULL, embedding VECTOR(1536), source_file_path TEXT, created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS precedents (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), source_id UUID REFERENCES cases(id), target_id UUID REFERENCES cases(id),
  citation_type TEXT CHECK (citation_type IN ('onay', 'bozma', 'atif', 'karsit', 'diger')), weight REAL DEFAULT 1.0, UNIQUE(source_id, target_id)
);

CREATE TABLE IF NOT EXISTS deontic_rules (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), statute_code TEXT NOT NULL, formal_expression TEXT NOT NULL,
  natural_language_tr TEXT NOT NULL, natural_language_en TEXT NOT NULL, source_article TEXT, is_active BOOLEAN DEFAULT TRUE
);

CREATE TABLE IF NOT EXISTS echr_judgments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), application_number TEXT UNIQUE NOT NULL, case_name TEXT NOT NULL, judgment_date DATE NOT NULL,
  articles TEXT[] NOT NULL, outcome TEXT CHECK (outcome IN ('violation', 'no_violation', 'struck_out', 'friendly_settlement')),
  full_text TEXT NOT NULL, embedding VECTOR(1536), hudoc_url TEXT, created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS echr_turkish_citations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), echr_id UUID REFERENCES echr_judgments(id), turkish_precedent_id UUID REFERENCES cases(id), citation_context TEXT
);
...
RETURNS TRIGGER AS $$ BEGIN
  ...
  VALUES (NEW.id, COALESCE(NEW.email,''), NEW.raw_user_meta_data->>'full_name', NEW.raw_user_meta_data->>'avatar_url', 'pending', FALSE);
  RETURN NEW;
...
CREATE OR REPLACE FUNCTION private.is_admin(uid uuid DEFAULT auth.uid()) RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (SELECT 1 FROM public.profiles WHERE id=uid AND role='admin' AND is_approved=true);
$$;
CREATE OR REPLACE FUNCTION private.is_approved_user(uid uuid DEFAULT auth.uid()) RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
...
INSERT INTO storage.buckets(id,name,public) VALUES ('legal-files','legal-files',false) ON CONFLICT(id) DO UPDATE SET public=false;
DROP POLICY IF EXISTS "Users upload own legal files" ON storage.objects;
CREATE POLICY "Users upload own legal files" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id='legal-files' AND (storage.foldername(name))[1]=(select auth.uid())::text AND (select private.is_approved_user()));
DROP POLICY IF EXISTS "Users read own legal files" ON storage.objects;
CREATE POLICY "Users read own legal files" ON storage.objects FOR SELECT TO authenticated USING (bucket_id='legal-files' AND ((storage.foldername(name))[1]=(select auth.uid())::text OR (select private.is_admin())));
DROP POLICY IF EXISTS "Users delete own legal files" ON storage.objects;
CREATE POLICY "Users delete own legal files" ON storage.objects FOR DELETE TO authenticated USING (bucket_id='legal-files' AND ((storage.foldername(name))[1]=(select auth.uid())::text OR (select private.is_admin())));

CREATE OR REPLACE FUNCTION public.bootstrap_admin(bootstrap_email text) RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE caller uuid := auth.uid(); existing_admin integer;
BEGIN
  ...
  UPDATE public.profiles SET role='admin', is_approved=true, approved_at=now(), approved_by=caller WHERE id=caller;
  IF NOT FOUND THEN RETURN false; END IF;
  INSERT INTO public.approval_logs(user_id,action,old_value,new_value,performed_by,note) VALUES(caller,'approve','pending','admin',caller,'Initial administrator bootstrap');
  RETURN true;
END; $$;
REVOKE ALL ON FUNCTION public.bootstrap_admin(text) FROM PUBLIC, anon;
