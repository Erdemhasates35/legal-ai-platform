...
async function requireAdminApi() {
  ...
  if (!profile.is_approved || profile.role !== "admin") {
    return { profile: null, response: NextResponse.json({ error: "ADMIN_REQUIRED" }, { status: 403 }) };
  }
...
export async function GET() {
  ...
  const { data, error } = await service.from("profiles").select("id,email,full_name,role,tier,is_approved,approved_at,created_at").order("created_at", { ascending: false });
...
export async function PATCH(request: Request) {
  ...
  const service = createServiceClient();
  const { data: target, error: targetError } = await service.from("profiles").select("id,role,tier,is_approved").eq("id", userId).maybeSingle();
  if (targetError || !target) return NextResponse.json({ error: "USER_NOT_FOUND" }, { status: 404 });

  if (action === "approve") {
    const { error } = await service.from("profiles").update({ is_approved: true, role: target.role === "pending" ? "user" : target.role, approved_at: new Date().toISOString(), approved_by: profile!.id }).eq("id", userId);
    if (error) return NextResponse.json({ error: "APPROVAL_FAILED" }, { status: 500 });
    const { error: auditError } = await service.from("approval_logs").insert({ user_id: userId, action: "approve", old_value: String(target.is_approved), new_value: "true", performed_by: profile!.id });
  ...
  if (action === "reject") {
    ...
    const { error: auditError } = await service.from("approval_logs").insert({ user_id: userId, action: "reject", old_value: String(target.is_approved), new_value: "false", performed_by: profile!.id });
    if (auditError) return NextResponse.json({ error: "AUDIT_FAILED" }, { status: 500 });
  }
  ...
  if (action === "tier_change" && tier) {
    ...
    const { error: auditError } = await service.from("approval_logs").insert({ user_id: userId, action: "tier_change", old_value: target.tier, new_value: tier, performed_by: profile!.id });
    if (auditError) return NextResponse.json({ error: "AUDIT_FAILED" }, { status: 500 });
  }
